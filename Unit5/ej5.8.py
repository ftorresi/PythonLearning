...
sunhours=np.array(data)

#Task 1

monthly_mean=np.zeros(sunhours.shape[1])   #sunhours.shape[0] == number of columns
monthly_mean=np.sum(sunhours,axis=0) #axis=0 means sum ACROSS rows, i.e. the sum will have the same number of elements as a row. The sum runs over row index for each fixed column index.
monthly_mean /= sunhours.shape[0]
print(monthly_mean)
...
#Task 2
max_value = np.max(monthly_mean)
month = month_names[np.argmax(monthly_mean)] #np.argmax to find the location of max. value in an array
print ("%s has best weather with %.1f sun hours on average" %(month, max_value))


#Task 3
#I could get rid of one loop, but there's another one that IDK how to avoid :(

decade_array=np.zeros((11,2))
for i in range(int(sunhours.shape[0]/10)): #number of decades
    decade_array=sunhours[10*i:11+10*i,[0,-1]] #years 1929+10*i;1939+10*i, months jan and dec
    decade_av=np.sum(decade_array[:,0])+np.sum(decade_array[:,1])-decade_array[0,0]-decade_array[10,1] #sum over january and december (:,0 and :,1) minus first jan and last dec.
    decade_av/=(10*2*30) #daily average
    print ("Decade %d-%d: %.1f" %(1930+i*10, 1939+i*10, decade_av))
