...
class IntervalMath(object):
    ...
    def __truediv__(self, other):  #python2's __div__ became __truediv__ or __floordiv__ in Python3
        a, b, c, d = self.lo, self.up, other.lo, other.up
        # [c,d] cannot contain zero:
        if c*d <= 0:
            raise ValueError('Interval %s cannot be denominator because it contains zero')
        return IntervalMath(min(a/c, a/d, b/c, b/d),
                            max(a/c, a/d, b/c, b/d))
...
class IntervalMath(object):
    ...
    def __rtruediv__(self, other):  #python2's __div__ became __truediv__ or __floordiv__ in Python3
        other = IntervalMath.n2i(other)
        return other/self
...
def _examples():
    ...
    y = v0*t - 0.5*t**2
    print( 'v0 =', v0, ', t =', t, ', y =', y)
    print ('mean y =%g, error=%.2f%%' %( float(y),0.5*y.width_in_percent()))
