...
class Polynomial:
    ...
    def __sub__(self, other):
        ...
        # If self is longer, no problem
        if len(self.coeff) >= len(other.coeff):
            result_coeff = self.coeff[:]  # copy!
            for i in range(len(other.coeff)):
                result_coeff[i] -= other.coeff[i] #substract other
        else: #if other is longer, we have to make some 0-coeff
            result_coeff = other.coeff[:] # copy (to get the same length)
            for i in range(len(other.coeff)):
                if i<len(self.coeff):
                    result_coeff[i] = self.coeff[i]-other.coeff[i] #make substraction where possible
                else: 
                    result_coeff[i] = - other.coeff[i] #change sign for terms of greater grade than self
        return Polynomial(result_coeff) #return a Polynomial (not a list of coeff.)
...
def test_Polynomial():
    ...
    p7=p1-p2
    p7_exact = Polynomial([1, -2, 0, 0, 6, 1])
    msg = 'p1 = %s, p2 = %s\np7=p1-p2 = %s\nbut wrong p7 = %s'%\
          (p1, p2, p7_exact, p7)
    assert p7.coeff == p7_exact.coeff, msg
    
    p8=p2-p1
    p8_exact = Polynomial([-1, 2, 0, 0, -6, -1])
    msg = 'p1 = %s, p2 = %s\np8=p2-p1 = %s\nbut wrong p8 = %s'%\
          (p1, p2, p8_exact, p8)
    assert p8.coeff == p8_exact.coeff, msg
