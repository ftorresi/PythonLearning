...
dt=0.001
n=int(T/dt)+1
tpoints=np.linspace(0,T,n+1) #We need to respect final point here

def straight(x):   #no need to define this, just to remember the target curve.
    return 0.5*x+1

def fstr(y,x):
    return np.sqrt(1+0.5**2)  #we could try calculating y'(x) numerically...

    
fEuler=ForwardEuler(fstr)
fEuler.set_initial_condition(0)
sol, t = fEuler.solve(tpoints)
print("Arc length of 0.5*x+1 from x=0 to x=2: %7.5f" %sol[-1])


def parabola(x): #no need to define this, just to remember the target curve.
    return x**2
...
fEuler=ForwardEuler(fpar)
fEuler.set_initial_condition(0) #curve length =0 on the starting point
sol, t = fEuler.solve(tpoints)
print("Arc length of x**2 from x=0 to x=2: %7.5f" %sol[-1])
