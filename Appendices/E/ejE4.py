"""This exercise aims to solve the ODE problem u'=f(u,t) given in command line"""
import numpy as np
...
n=int(T/dt)+1    
times=np.linspace(0,n*dt,n+1) #This way we respect the dt. The last point will be T' in [T;T+dt]


solver=method(f=f)
solver.set_initial_condition(U0=U0)
sol, t = solver.solve(times)
plt.plot(times, sol, "r--", label="%s dt=%g"%(solver.__class__.__name__,(t[1]-t[0])))


plt.legend()
plt.title("u'=%s; u(0)=%g"%(f_formula,U0))
plt.xlabel("t")
plt.ylabel("u(t)")
#plt.show()
plt.savefig("ejE4.png")
