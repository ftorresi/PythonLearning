...
rate=[]
for i in range(generations):
    previous_population=parents
    parents, mc = advance_generation(parents, 'one son',
                                     male_portion, fertility,
                                     law_breakers, wanted_children)
    r=(len(parents)/len(previous_population))-1 #growth rate of population
    rate.append(r)
    print ('%3d: %d (max children in a family: %d)' % \
          (i+1, len(parents), mc))
    print ('  Growth rate between generations: %5.3f' %r)

avg_r=100*sum(rate)/len(rate)
print ('Average growth rate between generations: %5.2f%%' %avg_r)
